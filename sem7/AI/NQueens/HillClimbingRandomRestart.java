...
public class HillClimbingRandomRestart {
    ...
    //Method to create a new random board //random row value but unique col value
    ...
    private static void printState (NQueen[] state) {
        ...
        for (int i=0; i<n; i++) {
            ...
            //System.out.println(state[i].getRow());
        }
        System.out.println();
        for (int i=0; i<n; i++) {
            for (int j= 0; j < n; j++) {
        ...
        for (int i = 0; i< state.length; i++) {
            for (int j=i+1; j<state.length; j++ ) {
                if (state[i].ifConflict(state[j])) {
                    heuristic++;					//heuristic value should be minimum possible
                }
            }
        }
        return heuristic;
    ...
    public static NQueen[] nextBoard (NQueen[] presentBoard) {
        ...
        for (int i=0; i<n; i++) {
            ...
            for (int j=0; j<n; j++) {
                ...
                //Move the queen
                if (tmpBoard[i].getRow()!=n-1) //not at bottom
                    tmpBoard[i].move();
            }
        }
